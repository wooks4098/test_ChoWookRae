...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		TransparentBlt(hdc, x, y, g_size.cx / 4, g_size.cy / 4, g_playerDC, 
								0, 0, g_size.cx / 4, g_size.cy / 4, RGB(255, 0, 255));
	...
	case WM_TIMER:
		...
		if (m_eState == PLAYER_STATE_JUMP)
		{
			...
			//x = m_ptJumpPos.x - 
			//	cosf(0.02f * m_dwJumpCount * PI) * 50;
			y = m_ptJumpPos.y - 
				sinf(0.02f * m_dwJumpCount * PI) * 100;
